...
def vec_xxT(x):
    x = utilities.colFromRow(x)  # take it as a column vector
    return numpy.dot(x, x.T).reshape(x.size ** 2)  # build vec(xxT)
...
def kFoldQLR(D, L):
    ...
    for p in PCA_valules:
        ...
        for prior in prior_values:
            ...
            for lamb in lamb_values:
                ...
                for i in range(K):
                    ...
                    DTRexp=numpy.apply_along_axis(vec_xxT, 0, DTR)
                    DTR=numpy.vstack([DTRexp, DTR])
                    
                    LTR = L[idxTrain]   
                    
                    DTEexp=numpy.apply_along_axis(vec_xxT,0,DTE)
                    DTE=numpy.vstack([DTEexp, DTE])
        ...
        #plots.plot_qlr(lamb_values, p, primSlist)
    ...
    with open('validationResults/quadraticLogisticRegression/results.txt', 'r+') as file:
        file.truncate(0)
        file.write("Results with Quadratic Logistic Regression"+ '\n'+ str(output)+ '\n')
