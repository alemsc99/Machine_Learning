...
def kFoldSvmP(D, L):
    ...
    for k in K_values:
        for coeff in coeff_values:
            ...
            for p in PCA_values:
                ...
                for c in C_values:
                    ...
                    for i in range(K_Fold):
                        ...
                        Ghat = ((numpy.dot(DTR.T, DTR)+coeff)**d)+k*k
                        ...
                        (x, f, dd) = scipy.optimize.fmin_l_bfgs_b(
                            lagr, approx_grad=False,  x0=x0, bounds=bound_list, factr=1.0)
    
                        
                        kernel=((numpy.dot(DTR.T, DTE)+coeff)**d)+k*k
                        
                        srs = numpy.sum(numpy.dot(x*Z, kernel), axis=0)
            ...
            plots.plot_svmP(C_values, k, coeff, primSlist)
    
    
    output = str(results)
    # save output txt file containing results for each model
    with open('validationResults/SVMPoly/results.txt', 'a') as file:
